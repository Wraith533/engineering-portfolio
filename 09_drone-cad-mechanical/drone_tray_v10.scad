// =====================================================================
//  Drone component tray  —  v10 "SLEEK + DUCTS x2 (wide jetson)".
//  Cube Orange (+Here4) / Herelink / Jetson / RFD900.  BASE + full LID.
//  v9 but the Jetson-side ducts are 3x wider (fore-aft) for the stiff
//  micro-HDMI run Herelink->Jetson. Duct width is per-station parametric.
//  v6 / v7 / v8 / v9 are all untouched.
//
...
echo(str("v10 SLEEK+DUCTSx2(wide jet)  L=", L, "  W ", W_end, "->", W_bulge, " (bulge)  top ", z_front, "/", z_rear));
...
// duct stations: xc = fore-aft centre, yw = local body half-width, dw = fore-aft width
hl_dyw  = W_bulge/2;                   // herelink ducts (on the bulge)
jet_dx  = 186; jet_dyw = W_end/2;      // jetson ducts (for the micro-HDMI)
jet_dw  = 3 * duct_dw;                 // 3x wider (William, v10)

module duct_outer(xc, yw, side, dw) {
    yo = yw + duct_out; y_in = yw - 4;
    mirror([0, side < 0 ? 1 : 0, 0]) union() {
        // horizontal run out from the side wall
        translate([xc - dw/2, y_in, duct_zc - duct_h/2]) cube([dw, yo - y_in, duct_h]);
        // vertical run dropping at the outboard end
        translate([xc - dw/2, yo - duct_h, duct_zc + duct_h/2 - duct_down]) cube([dw, duct_h, duct_down]);
    }
}
module duct_bore(xc, yw, side, dw) {
    yo = yw + duct_out; y_in = yw - 7; bx = dw - 2*duct_wall;
    mirror([0, side < 0 ? 1 : 0, 0]) union() {
        translate([xc - bx/2, y_in, duct_zc - duct_bore/2]) cube([bx, (yo - duct_wall) - y_in, duct_bore]);
        translate([xc - bx/2, yo - duct_h + duct_wall, duct_zc + duct_h/2 - duct_down - 2]) cube([bx, duct_bore, duct_down + 2]);
    }
}
...
module lid() {
    union() {
        difference() {
            union() {
                profiled(0);
                duct_outer(hl_cx,  hl_dyw,  1, duct_dw); duct_outer(hl_cx,  hl_dyw,  -1, duct_dw);
                duct_outer(jet_dx, jet_dyw, 1, jet_dw);  duct_outer(jet_dx, jet_dyw, -1, jet_dw);
            }
            ...
            duct_bore(hl_cx,  hl_dyw,  1, duct_dw); duct_bore(hl_cx,  hl_dyw,  -1, duct_dw);
            duct_bore(jet_dx, jet_dyw, 1, jet_dw);  duct_bore(jet_dx, jet_dyw, -1, jet_dw);
        }
