// =====================================================================
//  Drone component tray  —  v9 "SLEEK + DUCTS x2".  Cube Orange (+Here4) /
//  Herelink / Jetson / RFD900.   Single BASE + single full-length LID.
//  v8 + a SECOND pair of cable ducts on the Jetson sides (for the stiff
//  micro-HDMI run Herelink->Jetson). Ducts are parametric (any station).
//  v6 / v7 / v8 are all untouched.
//
...
echo(str("v9 SLEEK+DUCTSx2  L=", L, "  W ", W_end, "->", W_bulge, " (bulge)  top ", z_front, "/", z_rear));
...
// duct stations: xc = fore-aft centre, yw = local body half-width at that x
hl_dyw  = W_bulge/2;                   // herelink ducts (on the bulge)
jet_dx  = 200; jet_dyw = W_end/2;      // jetson ducts (rear rect, for the micro-HDMI)

module duct_outer(xc, yw, side) {
    yo = yw + duct_out; y_in = yw - 4;
    mirror([0, side < 0 ? 1 : 0, 0]) union() {
        // horizontal run out from the side wall
        translate([xc - duct_dw/2, y_in, duct_zc - duct_h/2]) cube([duct_dw, yo - y_in, duct_h]);
        // vertical run dropping at the outboard end
        translate([xc - duct_dw/2, yo - duct_h, duct_zc + duct_h/2 - duct_down]) cube([duct_dw, duct_h, duct_down]);
    }
}
module duct_bore(xc, yw, side) {
    yo = yw + duct_out; y_in = yw - 7;
    mirror([0, side < 0 ? 1 : 0, 0]) union() {
        translate([xc - duct_bore/2, y_in, duct_zc - duct_bore/2]) cube([duct_bore, (yo - duct_wall) - y_in, duct_bore]);
        translate([xc - duct_bore/2, yo - duct_h + duct_wall, duct_zc + duct_h/2 - duct_down - 2]) cube([duct_bore, duct_bore, duct_down + 2]);
    }
}
...
module lid() {
    union() {
        difference() {
            union() {
                profiled(0);
                duct_outer(hl_cx,  hl_dyw,  1); duct_outer(hl_cx,  hl_dyw,  -1);
                duct_outer(jet_dx, jet_dyw, 1); duct_outer(jet_dx, jet_dyw, -1);
            }
            ...
            duct_bore(hl_cx,  hl_dyw,  1); duct_bore(hl_cx,  hl_dyw,  -1);
            duct_bore(jet_dx, jet_dyw, 1); duct_bore(jet_dx, jet_dyw, -1);
        }
