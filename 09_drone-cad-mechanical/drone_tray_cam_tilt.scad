...
module full_lid(bo=0) {     // bo: 0 none | 1 windowed shroud | 2 solid full-gap shroud
    difference() {
        ...
        // rear-LEFT (-Y) bolt: drill through the WIDE jetson plate — the ear sits
        // INBOARD of the flared -Y edge, so the plate would otherwise refill it
        translate([blt_rx, -blt_ry, pod_top_z - 1]) cylinder(d=blt_clr_d, h=blt_head_h + 4);
        translate([blt_rx, -blt_ry, pod_top_z + 2]) cylinder(d=blt_head_d, h=blt_head_h + 1);
    }
}
