...
//  LAYOUT (fore-aft)
// ---------------------------------------------------------------------
// Jetson pod EXTENDED FORWARD into the herelink gap (2026-06-18).
// The bay was too short once the corner bolt bosses ate into it, so grow the
// pod jet_front_ext toward the herelink.  The REAR/tail face (jp_x0+jp_ol) is
// kept fixed -> RFD900, SMA holes and rear bolts don't move; only the front
// wall + jetson carrier shift forward, gaining jet_front_ext of rear clearance.
// (jet_front_ext = 10 consumes the 10mm rear gap, butting the herelink cover.)
jet_front_ext = 10;
hl_x0  = cc_x0 + cc_ol + gap;
jp_x0  = hl_x0 + hl_fore + gap - jet_front_ext;
...
blt_ry = rc_oy/2 + blt_out_r;       // rear bolts Y (±) — move in with the narrowed tail
...
else { tray(); jetbox_sq(); %jetbox_lid_lock(); %cam_lid(); %cube_lid(); %herelink_cover(); %mock(); }
