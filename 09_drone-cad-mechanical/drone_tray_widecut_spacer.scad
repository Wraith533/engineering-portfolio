...
blt_fx = cc_x0 + 6;                 // front bolts X (near the nose)
...
bo3_x0 = cc_ol;        // gap start (cube case rear)
...
module full_lid(bo=0) {     // bo: 0 none | 1 windowed shroud | 2 solid full-gap shroud
    difference() {
        union() {
            ...
            translate([cc_ol - wall, -cc_oy/2, lid_z])
                cube([wall, cc_oy, (pod_top_z + lid_t) - lid_z]);
            // --- gap bridge + JETSON section (high level) ---
            ybox(cc_ol - wall, jp_x0 - (cc_ol - wall), cc_oy, pod_top_z, lid_t);   // bridge
            ...
            // corner lid-bolt ears (2 front @ cube level, 2 rear @ jetson level)
...
module tray() { base_plate(); cube_case(); rfd_case(); front_bolt_posts(); rear_bolt_posts(); }
...
// =====================================================================
//  3/4-INCH SPACER RINGS (2026-07-01) — lift full_lid by spacer_h to open
//  interior room WITHOUT reprinting the tray/jetbox/lid.  Matches THIS
//  (widecut) revision's boxes: cube case + jetson pod, plain drop-rim seats
//  (no snap-lock — that was the 3lids_flush design the mismatched spacer came
//  from).  TWO SEPARATE rings (cube + jetson); they don't touch (herelink gap
//  between them) — print both flat on the plate.
//    * bottom drop-rim seats into the existing box mouth (== full_lid's rim)
//    * open tube top re-presents that same mouth so full_lid's rim drops in
//    * bolt bosses (front @ cube, rear @ jetson) carry the M3 clamp through
//  PRINT: part="spacer" (both) | "spacercube" | "spacerjet".
//  Front + rear M3 screws must be +spacer_h (19.05 mm) longer.
// =====================================================================
spacer_h = 19.05;   // 3/4 inch

// picture-frame drop-rim, identical to full_lid's, seating into a box mouth
module sp_rim(x0, len, w, top_z) {
    translate([x0 + wall + 0.4, -(w/2 - wall - 0.4), top_z - 3])
        difference() {
            cube([len - 2*wall - 0.8, w - 2*wall - 0.8, 3]);
            translate([1.6, 1.6, -1]) cube([len - 2*wall - 4, w - 2*wall - 4, 5]);
        }
}
// thin shelf bonding the rim's base to the tube wall (anti-float during print)
module sp_shelf(x0, len, w, top_z) {
    difference() {
        ybox(x0 + wall,     len - 2*wall,     w - 2*wall,     top_z,     wall);
        ybox(x0 + wall + 2, len - 2*wall - 4, w - 2*wall - 4, top_z - 1, wall + 2);
    }
}
// bolt pass-through boss (clearance bore), webbed to the nearest outer wall
module sp_boss(cx, cy, z0, edge_y) {
    difference() {
        union() {
...
module spacer_cube() {                 // rides between the cube case and the lid
    difference() {                     // open tube, mouth == cube case mouth
        ybox(cc_x0, cc_ol, cc_oy, lid_z, spacer_h);
        ybox(cc_x0 + wall, cc_ol - 2*wall, cc_oy - 2*wall, lid_z - 1, spacer_h + 2);
    }
    sp_rim(cc_x0, cc_ol, cc_oy, lid_z);        // bottom: drops into the cube case
    sp_shelf(cc_x0, cc_ol, cc_oy, lid_z);
    sp_boss(blt_fx,  blt_fy, lid_z,  cc_oy/2);  // front bolt bosses
    sp_boss(blt_fx, -blt_fy, lid_z, -cc_oy/2);
}

module spacer_jet() {                  // rides between the jetbox and the lid
    difference() {
        ybox(jp_x0, jp_ol, jp_oy, pod_top_z, spacer_h);
        ybox(jp_x0 + wall, jp_ol - 2*wall, jp_oy - 2*wall, pod_top_z - 1, spacer_h + 2);
    }
    sp_rim(jp_x0, jp_ol, jp_oy, pod_top_z);    // bottom: drops into the jetbox
    sp_shelf(jp_x0, jp_ol, jp_oy, pod_top_z);
    sp_boss(blt_rx,  blt_ry, pod_top_z,  jp_oy/2);  // rear bolt bosses
    sp_boss(blt_rx, -blt_ry, pod_top_z, -jp_oy/2);
}

// connect the two ring sections into ONE piece, bridging OVER the herelink gap
// (the cube bay seats spacer_h lower than the jetson bay, so the bridge steps up)
module spacer_bridge() {
    hlcov_top = base_t + hl_h + hc_channel + wall;   // herelink cover top (~44.4)
    brg_z0    = hlcov_top + 1.6;                      // bridge underside clears the cover
    top_z     = pod_top_z + spacer_h;                // meets the raised lid's bridge underside
    // step-riser wall closing the cube ring's rear (sits under the lid's own step riser)
    translate([cc_x0 + cc_ol - wall, -cc_oy/2, lid_z])
        cube([wall, cc_oy, spacer_h]);
    // two side rails spanning the gap to the jetson ring, ABOVE the herelink cover
    translate([cc_x0 + cc_ol, cc_oy/2 - wall, brg_z0])
        cube([jp_x0 - (cc_x0 + cc_ol), wall, top_z - brg_z0]);
    translate([cc_x0 + cc_ol, -cc_oy/2, brg_z0])
        cube([jp_x0 - (cc_x0 + cc_ol), wall, top_z - brg_z0]);
}

module spacer() { spacer_cube(); spacer_jet(); spacer_bridge(); }
...
else if (part == "spacer")     spacer();       // BOTH rings (print together)
else if (part == "spacercube") spacer_cube();  // cube ring only
else if (part == "spacerjet")  spacer_jet();   // jetson ring only
else if (part == "allspaced") {                // assembled w/ spacer (view-only)
    tray(); jetbox(); spacer(); herelink_cover();
    translate([0,0,spacer_h]) full_lid();
}
