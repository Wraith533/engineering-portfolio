// =====================================================================
//  Drone component tray  —  v8 "SLEEK + DUCTS".  Cube Orange (+Here4) /
//  Herelink / Jetson / RFD900.   Single BASE + single full-length LID.
//  v7 + cable-routing DUCTS off each side of the middle (out ~15, down
//  ~23), built into the top shell, opening into the herelink bay.
//  v6 (drone_tray.scad) and v7 (drone_tray_v7.scad) are untouched.
//
...
echo(str("v8 SLEEK+DUCTS  L=", L, "  W ", W_end, "->", W_bulge, " (bulge)  top ", z_front, "/", z_rear));
...
duct_yo = W_bulge/2 + duct_out;        // outboard face

module duct_outer(side) {
    mirror([0, side < 0 ? 1 : 0, 0]) union() {
        // horizontal run out from the bulge wall
        translate([hl_cx - duct_dw/2, 39, duct_zc - duct_h/2]) cube([duct_dw, duct_yo - 39, duct_h]);
        // vertical run dropping at the outboard end
        translate([hl_cx - duct_dw/2, duct_yo - duct_h, duct_zc + duct_h/2 - duct_down]) cube([duct_dw, duct_h, duct_down]);
    }
}
module duct_bore(side) {
    mirror([0, side < 0 ? 1 : 0, 0]) union() {
        translate([hl_cx - duct_bore/2, 36, duct_zc - duct_bore/2]) cube([duct_bore, (duct_yo - duct_wall) - 36, duct_bore]);
        translate([hl_cx - duct_bore/2, duct_yo - duct_h + duct_wall, duct_zc + duct_h/2 - duct_down - 2]) cube([duct_bore, duct_bore, duct_down + 2]);
    }
}
...
module lid() {
    union() {
        difference() {
            union() { profiled(0); duct_outer(1); duct_outer(-1); }
            ...
            // duct bores (also open the herelink bay wall)
            duct_bore(1); duct_bore(-1);
        }
